...
rmt_channel_t& operator++(rmt_channel_t& orig){
    orig = static_cast<rmt_channel_t>(orig + 1);
    return orig;
}


rmt_channel_t operator++(rmt_channel_t& orig, int){
    rmt_channel_t rVal = orig;
    ++orig;
    return rVal;
}
...
static size_t fgen_max_mem_blocks(rmt_channel_t channel)
{
    size_t sum = 0;
...
{
    size_t N;
    for (rmt_channel_t i = (rmt_channel_t)0; i < RMT_CHANNEL_MAX; i++) {
        rmt_channel_t ch = (rmt_channel_t)(RMT_CHANNEL_MAX-1-i);
        ...
        if (FREQ_CHANNEL[ch].state == FGEN_CHANNEL_FREE && mem_blocks <= N) {
            ...
            if (mem_blocks > 1) {
                ...
                for(rmt_channel_t j = (rmt_channel_t)(ch+1); j<ch+1+mem_blocks; j++) {
...
esp_err_t fgen_find_freq(double fout, double duty_cycle, fgen_info_t* fgen)
{
    ...
    ESP_LOGW(TAG, "%f %f %f", dNhigh, dNlow, duty_cycle);
...
/* -------------------------------------------------------------------------- */
extern "C"{
static esp_err_t fgen_allocate(const fgen_info_t* info, gpio_num_t gpio_num, fgen_resources_t* res)
{
    esp_err_t ret;
